...
public:
    ...
    // 4. Верхняя оценка хроматического числа (Жадный алгоритм)
    ...
    // 5. Диаметр графа (максимальное кратчайшее расстояние)
    ...
    // 6. Транзитивность графа (Отношение 3 * Треугольники / Триады)
    static double Transitivity(const Graph& g) {
        ...
        for (int v : vertices) {
            ...
            triads += d * (d - 1) / 2; // Количество путей длины 2 с центром в v
        }
    ...
    // 7. Количество точек сочленения (через классический DFS)
    ...
    // 8. Количество мостов (через генерацию случайных переменных)
...
private:
    // Вспомогательный DFS для точек сочленения
    ...
    // Вспомогательный DFS для рандомизированного алгоритма мостов
    static void dfsRandomBridges(const Graph& g, int v, int p, int d, std::set<int>& visited, 
        ...
        for (int u : g.neighbors(v)) {
            if (u == p) continue;
            if (visited.count(u)) {
                // Если это обратное ребро (идет вверх по дереву)
                if (depth[u] < depth[v]) { 
                    uint64_t weight = rng();
                    if (weight == 0) weight = 1; // Защита от случайного нуля
                    xor_sum[v] ^= weight;
                    xor_sum[u] ^= weight;
                }
            } else {
                // Если это ребро остовного дерева
                dfsRandomBridges(g, u, v, d + 1, visited, depth, xor_sum, bridges, rng);
                xor_sum[v] ^= xor_sum[u]; // Поднимаем XOR-сумму наверх
                // Если XOR-сумма поддерева равна 0, значит нет обратных ребер, покрывающих его
                if (xor_sum[u] == 0) {
                    bridges++;
                }
            }
